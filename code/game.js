...
var actorChars = {
  "@": Player,
  "o": Key, // A key will wobble up and down
  "=": Quicksand, "|": Quicksand, "v": Quicksand 
};
...
// Quick is initialized based on the character, but otherwise has a
// size and position
function Quicksand(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1);
  if (ch == "=") {
    // Horizontal quicksand
    this.speed = new Vector(2, 0);
  } else if (ch == "|") {
    // Vertical quicksand
    this.speed = new Vector(0, 2);
  } else if (ch == "v") {
    // Drip quicksand. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Quicksand.prototype.type = "quicksand";
...
Quicksand.prototype.act = function(step, level) {
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle quicksand by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches quicksand and the player hasn't won
  // Player loses
  if (type == "quicksand" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "key") {
    ...
    // If there aren't any keys left, player wins
